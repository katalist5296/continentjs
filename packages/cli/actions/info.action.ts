...
export class InfoAction extends AbstractAction {
  ...
  async displayNestInformation(): Promise<void> {
    ...
    console.info(chalk.green('[ContinentJS Platform Information]'));
  ...
  displayCliVersion(): void {
    console.info(chalk.green('[Continent CLI]'));
    console.info(
      'Continent CLI Version :',
      chalk.blue(
    ...
    Object.keys(dependencies).forEach((key) => {
      if (key.indexOf('@continentjs') > -1) {
        ...
        nestDependencies.push({
          name: `${key.replace(/@continentjs\//, '').replace(/@.*/, '')} version`,
