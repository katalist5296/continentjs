...
const askForMissingInformation = async (inputs: Input[]) => {
  ...
  if (!nameInput!.value) {
    ...
    const questions = [generateInput('name', message)('continent-project')];
...
const generateApplicationFiles = async (args: Input[], options: Input[]) => {
  ...
  await collection.execute('project', schematicOptions);
  console.info();
};
...
const printCollective = () => {
  ...
  emptyLine();
  yellow(`Thanks for installing ContinentJS ${EMOJIS.PRAY}`);
  emptyLine();
};
