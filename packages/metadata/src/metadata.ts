...
const META_PREFIX = "@continentjs";
const META_NAME = `${META_PREFIX}:name`;
const META_UUID = `${META_PREFIX}:uuid`;
const META_ID = `${META_PREFIX}:id`;
...
export function isDecorator(decorator: Function): boolean {
  return isTruthy(decorator[META_ID]);
}
/**
 * Get Decorator id
 * @param decorator
...
function mapKeyToMetadata(key: IDecoratorMetadataKey): IMetadata {
  ...
  if (key.metadataKey.includes(META_PREFIX)) {
    return metadata;
  }
  return <IMetadata> {
    args: metadata,
...
function decorate(decorator: Function, type: string, args: object): any {
  ...
  if (isFalsy(decorator[META_ID])) {
    decorator[META_UUID] = uuid();
    decorator[META_ID] = `${META_PREFIX}:${type}:${decorator.name}:${decorator[META_UUID]}`;
    decorator[META_NAME] = decorator.name;
    decorator[META_TYPE] = type;
  }
